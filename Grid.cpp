...
void Grid::RemoveObjectFromCell(const CellPosition & pos)
{
	if (pos.IsValidCell()) // Check if valid position
	{
		...
		{
			CellList[pos.VCell()][pos.HCell()]->GetGameObject()->DestructCard();
		}
...
Card* Grid::HasCardInGrid(CellPosition & Sent)
{
	int v = Sent.VCell();
	int h = Sent.HCell();
...
/*Snake * Grid::HasSnakeInGrid(CellPosition & Sent)
{
int v = Sent.VCell();
int h = Sent.HCell();
...
Ladder * Grid::HasLadderInGrid(CellPosition & Sent)
{
int v = Sent.VCell();
int h = Sent.HCell();
...
void Grid::BuyThisCard(Card * card)
{
	for (int i = 0; i < NumVerticalCells; i++)
	{
		for (int j = 0; j < NumHorizontalCells; j++)
		{
			Card*Check = CellList[i][j]->HasCard();
			if (Check)
			{
				int compare = card->GetCardNumber();
				if (compare == Check->GetCardNumber())
				{
					Check->SetBought();
				}
			}
